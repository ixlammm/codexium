...
Import `cua`, list apps, and select a window returned by `cua`:

```js
globalThis.cua ??= (await import("@oai/cua")).cua;
globalThis.cua_init = await globalThis.cua.initialize();
globalThis.targetWindows = await cua.computer.list_windows();
globalThis.targetWindow = targetWindows.find((window) =>
  /chrome/i.test(String(window.app) + " " + String(window.title ?? "")),
);
if (!targetWindow) throw new Error("window not found");
globalThis.targetWindow = await cua.computer.get_window(targetWindow);
globalThis.state = await cua.computer.get_window_state({
  window: targetWindow,
...
nodeRepl.write(
  JSON.stringify({
    ...
    text: String(state.accessibility?.document_text ?? "").slice(0, 1200),
    screenshots: (state.screenshots ?? []).map((s) => ({ id: s.id })),
  }),
);
```

Do not split that first Windows call.
Read the discovered window directly; do not call activate_window before the first get_window_state.
Use `cua_init.apps` to discover or launch apps, then select an already-open target from `cua.computer.list_windows()` before the first snapshot.
Keep the initialized client alive across calls and use only the documented `cua.computer` methods.
Reuse the latest state.window; for coordinates from a prior closed call, omit the stale screenshot id and pass window-relative x/y.
...
await cua.computer.click({ window: targetWindow, element_index: 12 });
await cua.computer.click({
  window: targetWindow,
  screenshotId,
  x: 400,
  y: 300,
});
await cua.computer.set_value({
  window: targetWindow,
  element_index: 12,
  value: "hello",
});
await cua.computer.type_text({ window: targetWindow, text: "hello" });
await cua.computer.press_key({ window: targetWindow, key: "Return" });
await cua.computer.scroll({
  window: targetWindow,
  screenshotId,
  x: 400,
  y: 300,
...
await cua.computer.perform_secondary_action({
  window: targetWindow,
  element_index: 12,
  action: "Expand",
});

state = await cua.computer.get_window_state({
  window: targetWindow,
  include_screenshot: true,
  include_text: true,
});
...
- If a stored window stops working, recover with `cua.computer.list_windows()`, `cua.computer.get_window({ id: targetWindow.id, app: targetWindow.app })`, `cua.computer.activate_window({ window: targetWindow })`, then `cua.computer.get_window_state({ window: targetWindow, include_screenshot: true, include_text: true })`.
- If you expect a modal in the target app but `get_window_state` does not show it, call `cua.computer.list_windows()` to find the modal or owned secondary window, then capture that returned window with `cua.computer.get_window_state(...)` to obtain its accessibility state.
- If an input call reports that the point is over `StartMenuExperienceHost.exe` or another non-target window, call `cua.computer.activate_window({ window: state.window })`, refresh screenshot-backed state, and retry the intended input once with the refreshed `state.window`.

## API Reference

Use this as the supported `cua.computer` window2 API surface.

```ts
import { cua } from "@oai/cua";

const apps = await cua.computer.list_apps();

## node_repl + cua.computer (Computer Use)
...
```js
var state = await cua.computer.get_app_state({ app: "com.google.Chrome" });
...
```js
var apps = await cua.computer.list_apps();
...
```js
await cua.computer.click({ app: "Google Chrome", element_index: 42 });
await cua.computer.set_value({
  app: "Google Chrome",
  element_index: 42,
  value: "openai.com",
});
await cua.computer.press_key({ app: "Google Chrome", key: "Return" });
await cua.computer.type_text({ app: "Google Chrome", text: "hello" });
...
await cua.computer.select_text({
  app: "Google Chrome",
  element_index: 42,
  text: "hello",
});
...
nodeRepl.write((await cua.computer.get_app_state({ app: "Google Chrome" })).text);
```

or you can use the bundle id instead of the name, for example:

```js
await cua.computer.click({ app: "com.google.Chrome", element_index: 42 });
```

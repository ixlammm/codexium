# Computer Full Desktop API

Empty `apps` from `cua.initialize` is normal.

```ts
import { cua } from "@oai/cua";

let screenshot_list = await cua.computer.get_screenshot();
for (const screenshot of screenshot_list) {
  await nodeRepl.emitImage(screenshot.data_url);
}
```

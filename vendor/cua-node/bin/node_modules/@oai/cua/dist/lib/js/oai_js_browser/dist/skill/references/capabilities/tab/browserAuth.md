...
- Never include secrets, cookies, full URLs, query strings, JavaScript, DOM
  snippets, or other page content in a browser-auth request.
  Browser-client supplies the request message and derives field labels from the
  verified form; do not provide either.
...
- `declined` with `reason: "user_took_over"` means the user took manual control
  ...
  unknown. Inspect the final page state with fresh visible DOM and a
  screenshot, then continue the Authentication Lifecycle from its
  transition-inspection step. Do not inspect or act on any intermediate state
  from the manual sign-in.
